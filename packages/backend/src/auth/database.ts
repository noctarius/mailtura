...
export const mailturaAdapter = (config: MailturaAdapterConfig = {}) => {
  ...
  const createDatabaseAdapter =
    ...
    ({ getFieldName }) => {
      ...
      const operatorToPrismaOperator = (operator: string) => {
        switch (operator) {
          case "starts_with":
            return "startsWith";
          case "ends_with":
      ...
      const convertWhereClause = (model: string, where?: Where[] | undefined) => {
        ...
        const buildSingleCondition = (w: Where) => {
          ...
          if ((w.operator === "in" || w.operator === "not_in") && Array.isArray(w.value)) {
            ...
            if (filtered.length === 0) {
              if (w.operator === "in") {
                return {
                  AND: [{ [fieldName]: { equals: "__never__" } }, { [fieldName]: { not: "__never__" } }],
                };
              } else {
                return {};
              }
  ...
  adapterOptions = {
    config: {
      adapterId: "mailtura-adapter",
      adapterName: "Mailtura Database Adapter",
      usePlural: false,
